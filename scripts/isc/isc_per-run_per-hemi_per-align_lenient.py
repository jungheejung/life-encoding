...
__author__ = "Heejung Jung, Sam Nastase"
...
data_dir = '/Volumes/life_data' # /dartfs/rc/lab/D/DBIC/DBIC/life_data/
niml_dir = join(data_dir, 'niml')
gifti_dir = join(data_dir, 'life_dataset')
suma_dir = '/Users/h/suma-fsaverage6'
output_dir = join('/Volumes/life-encoding', 'scripts', 'isc', 'upperbound')
runs = [1, 2, 3, 4]
...
for run in runs:
    ...
    for subject_ind, subject in enumerate(subjects):
        ...
        other_avg = np.mean(cortical_data, axis=0)
        ...
        np.save(file = join(output_dir, f"subjectwise-ISCupperbound_align-{align}_hemi-{hemisphere}_run-{run:02d}_{subject}.npy"), 
        arr = result_gifti) # save subject wise ISC
        write_gifti(result_gifti.astype('float32'),
        # template_fn = join(suma_dir, f'{hemisphere}.pial.gii'), 
        output_fn = join(output_dir,f'subjectwise-ISCupperbound_align-{align}_hemi-{hemisphere}_run-{run:02d}_{subject}.gii'))
        isc_result[subject_ind] = result_gifti[0]
        # TODO: save to result_gifti
    # runwise ISC (numpy & average gifti)
    np.save(file= join(output_dir,f'runwise-ISCupperbound_align-{align}_run-{run:02d}_hemi-{hemisphere}.npy'), arr = isc_result)
    mean_runwise_isc = np.mean(isc_result, axis = 0)
    write_gifti(mean_runwise_isc.astype('float32'),
        # template_fn = join(suma_dir, f'{hemisphere}.pial.gii'), 
        output_fn = join(output_dir,f'groupaverage_runwise-ISCupperbound_align-{align}_run-{run:02d}_hemi-{hemisphere}.gii'))
...
for run in runs:
    r = np.load(file= join(output_dir,f'runwise-ISCupperbound_align-{align}_run-{run:02d}_hemi-{hemisphere}.npy'))
    np.stack((total_run, r))
    mean_isc = np.mean(total_run, axis = 0)
    write_gifti(mean_isc.astype('float32'),
        # template_fn = join(suma_dir, f'{hemisphere}.pial.gii'), 
        output_fn = join(output_dir,f'groupaverage-ISCupperbound_align-{align}_hemi-{hemisphere}.gii'))
