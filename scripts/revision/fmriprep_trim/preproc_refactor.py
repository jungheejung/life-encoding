...
# for idx, row in video_events.iterrows():
    ...
    final_lh_path = f'{sub_output_dir}/{SUB}_{base_stimfile}_lh.func.gii'
    # output_dir / f"{base_name}_lh_fsaverage.func.gii"
    final_rh_path = f'{sub_output_dir}/{SUB}_{base_stimfile}_rh.func.gii'
    
    final_fsaverage_lh_path = f'{sub_output_dir}/{SUB}{base_stimfile}_lh_fsaverage6.func.gii'
    final_fsaverage_rh_path = f'{sub_output_dir}/{base_stimfile}_rh_fsaverage6.func.gii'
    
    resampled_cifti_path = {sub_output_dir} / f"{SUB}_{base_name}_fsaverage.dtseries.nii"
    ...
    #  -gifti-convert ASCII 
    ascii_lh_path = f'{sub_output_dir}/{SUB}_{base_stimfile}_lh_ascii.func.gii'
    ASCII_command = ['wb_command', '-gifti-convert', 'ASCII', str(final_lh_path), str(ascii_lh_path)]


    subprocess.run(ASCII_command, check=True)

    ascii_rh_path = f'{sub_output_dir}/{SUB}_{base_stimfile}_rh_ascii.func.gii'
