...
# %% 4 fold CV with 3 videos left out. 
...
# NOTE [x] need to add lags
# NOTE [x] Tr * 5 10, 15, 20
...
# hyper_dir = os.path.join(data_dir, 'hyperalign_mapper') # NOTE:DEP:
# Life fMRI data directory (/idata/DBIC/snastase/life)
# fmri_dir = os.path.join(data_dir, 'life_dataset')
fmri_dir = '/dartfs/rc/lab/H/HaxbyLab/heejung/data_spacetoptrim'
...
model_dir = '/dartfs/rc/lab/H/HaxbyLab/heejung/data/annotations/glove'
...
model_ndim = 300
run_labels = [1, 2, 3, 4] # NOTE: HOW TO CHANGE
...
match = re.search(r'\d+', test_subject)
if match:
    sub_number = int(match.group())
...
test_index = cv_list[test_run_id][1]

train_runs = np.array(list(model_durs.keys()))[train_index]
test_runs = np.array(list(model_durs.keys()))[test_index]
...
# NOTE [x] shuffle the video order. CV scheme within each subject.
...
def load_ws_data(test_subject, test_runs, train_runs,
...
# train_ids = []
# for i, train_run in enumerate(train_runs):
#    train_ids.extend([i] * model_durs[train_run])
