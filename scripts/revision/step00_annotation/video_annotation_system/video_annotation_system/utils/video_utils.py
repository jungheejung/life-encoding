...
def validate_input(video_path: str) -> bool:
    ...
    try:
        ...
        if not cap.isOpened():
            ...
            # Try with av (PyAV) as a fallback
            try:
                with av.open(video_path) as container:
                    logger.info(f"Successfully opened video with av: {video_path}")
                    return True
            except Exception as e:
                logger.error(f"Could not open video with av: {str(e)}")
                return False
...
def is_video_file(file_path: str) -> bool:
    ...
    return Path(file_path).suffix.lower() in get_supported_video_formats()
