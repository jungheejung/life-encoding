...
directory = os.path.join(scratch_dir, 'PCA_banded-ridge_loro_fullrange_nochunk',
...
out_hyperparam = np.zeros(
    (alpha_shape + med_wall_ind.shape[0]), dtype=np.dtype(new_alphas).type)
out1[cortical_vertices[hemi] == 1] = new_alphas

# 6-4. save alpha
np.save(os.path.join(directory, 'hyperparam-alpha_{0}_model-{1}_align-{2}_foldshifted-{3}_hemi-{4}_range-{5}.npy'.format(
        test_p, model, align,  fold_shifted, hemi,save_nodename)), outhyperparam)
...
mv.niml.write(os.path.join(directory, 'corrcoef_{0}_model-{1}_align-{2}_feature-{3}_foldshifted-{4}_hemi_{5}.npy'.format(
...
mv.niml.write(os.path.join(directory, 'corrcoef_{0}_model-{1}_align-{2}_feature-{3}_foldshifted-{4}_hemi_{5}.npy'.format(
    test_p, model, align, stimfile3, fold_shifted, hemi)), outtotal[None, :])
# copy files and remove files ___________________________________________________________________
subprocess_cmd('cp -rf /scratch/f0042x1/PCA_banded-ridge_loro_fullrange_nochunk/ /dartfs/rc/lab/D/DBIC/DBIC/f0042x1/life-encoding/results; rm -rf /scratch/f0042x1/*')
print("\nprocess complete")
