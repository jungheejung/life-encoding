...
for p in range(num_parcels):
	data[p,:]=raw_data[np.where(parcellation==p+1)].mean(axis=0)   # AVERAGE OF THE PARCELS

# TODO: CHECK IF THIS IS USED OR NOT IN HIMALAYAS
...
X1=X0_full[:,~is_silent_features]


Y=data.T 
# INNER OUTER 3 CV
skf_outer=KFold(n_splits=3,shuffle=False)
skf_inner=KFold(n_splits=3,shuffle=False) # TODO: DOUBLE CHECK IF THIS skf_inner IS THE EQUIVALENT TO OUR LORO SCHEME
performances=np.zeros((1000,3)) # TODO: CHECK 3 - BANDS?
test_idx=0
...
for train_index,test_index in skf_outer.split(X):
	...
	# TODO: Z-SCORE. SKLEARN. REMOVE MEAN FROM TEST, USING THE TRAIN MEAN
	
	# TODO: fit_intercept >> IF WE DO SKLEARN ZSCORING, fit_intercept=False
	# n_iter, MORE ITERATIONS, LOWER ERROR
	# TODO: WHAT'S THE SWEET SPOT OF n_iter. MORE BANDS, MORE ITER? IF SO, HOW MUCH FOR 3 BANDS?
	model=GroupRidgeCV(groups="input",cv=skf_inner,fit_intercept=False,solver_params=dict(score_func=correlation_score,progress_bar=True,n_iter=1000))
	model.fit([X0_train,X1_train,X_train],Y_train) 
	# TODO: Y_train TAKE ALL THE VOXELS, NOT VOXELWISE
	# TODO: Y SHAPE?

	Y_pred=model.predict([X0_test,X1_test,X_test],split=True)[2]
	#print(Y_pred.shape)
	for p in range(1000): 
		performances[p,test_idx]=pearsonr(Y_pred[:,p],Y_test[:,p])[0]
