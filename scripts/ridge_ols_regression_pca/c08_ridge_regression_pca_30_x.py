...
npy_dir = '/dartfs/rc/lab/D/DBIC/DBIC/f0042vj/life-encoding/w2v_features'
...
def OLS_regression(train_stim, train_resp, test_stim, test_resp):
	train_stim = np.concatenate(train_stim, axis=0)
	train_resp = np.concatenate(train_resp, axis=0)
	print(train_stim.shape)
	print(train_resp.shape)
	ols_model = LinearRegression()
	ols_model.fit(train_stim,train_resp)
	wt = ols_model.coef_

	train_pred = ols_model.predict(train_stim)
	test_pred = ols_model.predict(test_stim)

	#resvar = (test_resp-test_pred).var(0)
	#Rsqs = 1 - (resvar / test_resp.var(0))
	#corrs = np.sqrt(np.abs(Rsqs)) * np.sign(Rsqs)

	corrs = np.nan_to_num(np.array([np.corrcoef(test_resp[:,ii], test_pred[:,ii].ravel())[0,1]
										for ii in range(test_resp.shape[1])]))

	return wt, corrs, train_pred, test_pred
...
for hemi in hemispheres:
	for fold in fold_exclude:
		...
		if model == 'visual':
			train_stim, test_stim = get_visual_stim_for_fold('{0}_{1}'.format(model, stimfile), fold_shifted, included, pca_or_not)
			print("test_stim row:",len(test_stim))
			print("test_stim col:",len(test_stim[0]))
		else:
		...
		for test_p in participants:
			if align == 'ws':
				...
				print("test_resp row:",len(test_resp))
				print("test_resp col:",len(test_resp[0]))
			elif align == 'aa':
			...
			if ridge_or_not == True:
				...
				ols_model, corrs, train_pred, test_pred = OLS_regression(train_stim, train_resp, test_stim, test_resp)
			...
			print('\n\nWriting to file...')
			directory = os.path.join('/dartfs-hpc/scratch/f0042vj/life-encoding/results/ridge-models-pca-30-x', '{0}/{1}/{2}/leftout_run_{3}'.format(align, model, stimfile, fold_shifted), test_p, hemi)
