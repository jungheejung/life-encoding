...
alignment="aa" # ws, aa, ha_common, ha_test
features="bg actions agents"

echo ${subject} ${hemisphere} ${test_run} ${roi}
python ${MAINDIR}/banded_ridge.py -a ${alignment} --hemisphere ${hemisphere} --test-run ${test_run} -s ${subject} -f ${features} --roi ${roi}
