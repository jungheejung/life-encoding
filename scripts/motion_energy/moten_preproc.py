...
for run in [1,2,3,4]:
    ...
    number_of_samples = round(moten_log.shape[0] * fmri_rate / fps)
    resample_scipy = signal.resample(moten_log, number_of_samples, axis = 0)
    ...
    # hstack _____________________________________________________________________________
...
np.sum(model_durs.values()) == resample_all.shape[0]
