...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks,
    ...
    print('train_stim: {0}'.format(Rstim.shape))
    print('train_resp: {0}'.format(Rresp.shape))
    print('test_stim: {0}'.format(Pstim.shape))
    print('test_resp: {0}'.format(Presp.shape))
    ...
    #     logger.info("Selecting held-out test set..")
    #
    #     RRstim = np.concatenate((stim[:k] + stim[(k + 1):])) # training
    #     PRstim = stim[k] # test
    #     RRresp = np.concatenate((resp[:k] + resp[(k + 1):]))
    #     PRresp = resp[k]
