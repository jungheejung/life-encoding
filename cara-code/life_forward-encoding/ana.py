...
# '/ihome/cara/life/w2v_src/google_w2v_ca.npy'
    ...
    for hemi in hemispheres:
        ...
        for test_p in participants:
            ...
            for run in [1,2,4]:
                ...
                for participant in train_p:
                    if run == 4:
                        resp = load_data(os.path.join(sam_data_dir, '{0}_task-life_acq-346vol_run-02.{1}.tproject.gii'.format(participant, hemi))).samples[4:-12,:]
                    else:
                        resp = load_data(os.path.join(sam_data_dir, '{0}_task-life_acq-{1}vol_run-0{2}.{3}.tproject.gii'.format(participant, tr[run], run, hemi))).samples[4:-7,:]
            ...
            test_resp = load_data(os.path.join(sam_data_dir, '{0}_task-life_acq-{1}vol_run-0{2}.{3}.tproject.gii'.format(test_p, tr[3], 3, hemi))).samples[4:-7,:]
            ...
            print('\n\nWriting to file...')
            directory = os.path.join(ridge_dir, 'avg-{}-ana-CV'.format(stimfile[:-4]), test_p, hemi)
        ...
        np.save(os.path.join(ridge_dir, 'avg-{}-ana-CV'.format(stimfile[:-4]), 'avg_corrs.{0}.npy'.format(hemi)), m)
