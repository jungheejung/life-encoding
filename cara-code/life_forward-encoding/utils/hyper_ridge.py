# from https://github.com/gallantlab/ieee-hackathon/tree/master/src/ridge
...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots,
                    ...
                    nuisance_regressor=False, use_corr=True, logger=ridge_logger):
    ...
    nresp, nvox = np.concatenate(Rresp).shape
    ...
    for k in range(nboots):
        ...
        print('RRstim: {0}'.format(RRstim.shape))
        print('PRstim: {0}'.format(PRstim.shape))
        print('RRresp: {0}'.format(RRresp.shape))
        print('PRresp: {0}'.format(PRresp.shape))
    ...
    if not single_alpha:
        ...
        if nboots==0:
            ...
            # allRcorrs shape: (A alphas, M surface nodes, B nboots)
    ...
    Rstim = np.concatenate(Rstim)
    Rresp = np.concatenate(Rresp)
