...
def make_afni_stimtimes_exemplar(in_files, start=0):
        ...
        for row in events.itertuples():
            ...
            if row.Index == 0:
                ...
                if row.response == 'same':
                    onsets['same_response'].append("{0:.2f}".format(row.onset +
                                                   row.reaction_time))
        onsets = dict(onsets)
        return onsets
...
def init_datasource_wf(subject, task, space, name='datasource_wf'):
    ...
    **Outputs**
    ...
    if space == 'fsaverage6':
        out_ext = 'gii'
    elif 'hpal' in space and 'fsaverage6' in space:
        out_ext = 'niml.dset'
    else:
        surface = False
        out_ext = 'nii.gz'
    ...
    def filter_hemisphere(in_files, hemi):
        return list(filter(lambda x: hemi + '.func.' in x, in_files))
    ...
    if tr is not None:
        truncate_times = MapNode(
            TimingToolPy(truncate_times=True, tr=tr,
                 py27_path='/opt/conda/bin/python'),
...
def init_afnipreprocess_wf(blurfwhm=4.0, name='afnipreprocess'):
    ...
    **Outputs**
    ...
    def merge_mat(motion_params, nuisance_regressors):
        import numpy as np
        import os
...
def init_glm_localizer_wf(subject, space='fsaverage6', smooth=None,
    ...
    **Outputs**
    ...
    # AFNI model
    out_ext = 'niml.dset' if space == 'fsaverage6' else 'nii.gz'
    afnimodel_wf = init_afnimodel_wf(blurfwhm=None,
                                     out_ext=out_ext,
                                     njobs=njobs,
                                     do_remlfit=True,
